...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		String beanName = id;
		// 如果没有指定id属性，使用bean的name属性作为注入的名字
		...
		/**
		 * 解析其它属性 实际类型为子类{@link org.springframework.beans.factory.support.GenericBeanDefinition}
		 * */
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			/**
			 * 创建用于承载属性的 {@link org.springframework.beans.factory.support.GenericBeanDefinition}
			 * */
			...
			// 获取器注入
			/**
			 * 示例见 {@link org.fade.demo.springframework.beans.Main#testLookupMethod()}
			 * */
			parseLookupOverrideSubElements(ele, bd.getMethodOverrides());
			// 解析replaced-method子元素
			/**
			 * <p>可在运行时用新的方法替换现有的方法，不仅可以动态的替换返回实体bean
			 * 还能动态更改原有方法的逻辑</p>
			 * */
			...
			// 解析property子元素 setter注入
			parsePropertyElements(ele, bd);
			// 解析qualifier子元素
			// qualifier子元素与@Qualifier作用一样
	...
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
		...
		// 如果父标签没有scope属性，则取子bean标签的scope属性值
	...
	public void parseLookupOverrideSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, LOOKUP_METHOD_ELEMENT)) {
				...
				/**
				 * <p>{@link LookupOverride} 用于承载数据，并把其
				 * 记录在 {@link AbstractBeanDefinition#methodOverrides} 属性中</p>
				 * */
				LookupOverride override = new LookupOverride(methodName, beanRef);
				override.setSource(extractSource(ele));
				/**
				 * 也是记录在了 {@link AbstractBeanDefinition#methodOverrides} 属性中
				 * */
	...
	public void parseConstructorArgElement(Element ele, BeanDefinition bd) {
		...
		if (StringUtils.hasLength(indexAttr)) {
				...
				if (index < 0) {
					...
					try {
						...
						if (bd.getConstructorArgumentValues().hasIndexedArgumentValue(index)) {
							...
							/**
							 * 如果指定了index属性，则将承载数据的{@link org.springframework.beans.factory.config.ConstructorArgumentValues.ValueHolder}
							 * 存放至indexedArgumentValues中
							 * */
			...
			try {
				...
				valueHolder.setSource(extractSource(ele));
				/**
				 * 如果没有指定index属性，则将承载数据的{@link org.springframework.beans.factory.config.ConstructorArgumentValues.ValueHolder}
				 * 添加至genericArgumentValues中
				 * */
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		if (hasRefAttribute) {
			...
			/**
			 * 用 {@link RuntimeBeanReference} 封装信息
			 * */
